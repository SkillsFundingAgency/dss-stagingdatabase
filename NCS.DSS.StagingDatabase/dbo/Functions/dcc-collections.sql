   ...
   INSERT INTO @Result
	...
	from (
			...
			OUTER APPLY			(	SELECT TOP 1	PostCode
									...
									WHERE			a.CustomerId = o.CustomerId	 	-- Get the latest address for the customer record
			...
			AND					o.id NOT IN (
												...
												WHERE			(
																	(	-- if sustained employment check that there are no other outcomes of the same type in the last 13 months
																		...
																		AND 
																		priorO.OutcomeEffectiveDate > DATEADD(mm, -13, o.OutcomeEffectiveDate)
																	)
																	OR
																	(	-- if NOT sustained employment check that there are no other outcomes of the same type in the last 12 months
																		o.OutcomeType <> 3
																		AND 
																		priorO.OutcomeEffectiveDate > DATEADD(mm, -12, o.OutcomeEffectiveDate)
																	)
																)
												AND
																(
												...
												AND				priorO.id <> o.id							-- and are not the same ID
