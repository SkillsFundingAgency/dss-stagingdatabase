...
CREATE VIEW [PowerBI].[v-dss-pbi-profile-total] AS 
    SELECT 
        MY.[PeriodYear]
        ,MY.[PeriodMonth]
        ,MY.[CMO_Num] AS [CustomerCount]
        ,SUM(MY.[CMO_Num]) OVER(PARTITION BY MY.[PeriodYear] ORDER BY MY.[PeriodMonth]) AS [YTD_CustomerCount] 
    FROM 
    (
        SELECT 
