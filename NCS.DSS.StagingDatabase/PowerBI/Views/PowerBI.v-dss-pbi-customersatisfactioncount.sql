...
AS 
    WITH RelevantData AS (
             SELECT 
                ...
                CASE 
                    WHEN IIF((ep.EconomicShockStatus = 2) AND (ep.EconomicShockCode LIKE 'Aspen%'), 1, IIF(p.PriorityGroup IN (1,2,3,4,5,6), 1, 0)) = 1 THEN 'PG'
                    ELSE 'NP'
        ...
        CASE WHEN MONTH(AP.DateActionPlanCreated) < 4 THEN CONCAT(YEAR(AP.DateActionPlanCreated) - 1, '-', YEAR(AP.DateActionPlanCreated)) ELSE CONCAT(YEAR(AP.DateActionPlanCreated), '-', YEAR(AP.DateActionPlanCreated) + 1) END AS PeriodYear,
                ...
                RANK() OVER (PARTITION BY ap.CustomerID, FY.FinancialYear ORDER BY ap.DateActionPlanCreated, ap.LastModifiedDate, p.PriorityGroup, RIGHT(ap.CreatedBy, 3)) AS Rank
			...
			LEFT JOIN [dbo].[dss-employmentprogressions] ep on ep.CustomerId = c.id AND ep.DateProgressionRecorded BETWEEN DATEADD(MONTH, -12, ap.DateActionPlanCreated) AND ap.DateActionPlanCreated
     ...
     SELECT 
            ...
            FROM RelevantData AS R
			    ...
			    AND ((Age >= 19) OR (PriorityCustomer = 1 AND Age >= 18 AND Age <= 24))
